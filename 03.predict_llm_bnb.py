...
# 모델 로딩 (lora)
model = PeftModel.from_pretrained(model, '/home/jspark/projects/FUse/LLM/script/output/llama-8B-model_1122', torch_dtype= torch.bfloat16, device_map = 'auto')
...
def make_prediction(prompt):
    ...
    generate_ids = model.generate(tokenized_chat,
    max_new_tokens=256,  # 적절한 최대 길이 제한
    temperature=0.8,  # 다양성을 줄이기 위해 낮게 설정
    top_k=50,  # 상위 50개 후보 중에서 선택
    top_p=0.9,  # 누적 확률이 0.9인 후보들만 고려
...
predicted_data.to_excel('/home/jspark/projects/FUse/LLM/script/predictions/1122-8B/prediction.xlsx', index=False)
top_3_predicted_data.to_excel('/home/jspark/projects/FUse/LLM/script/predictions/1122-8B/top_3_prediction.xlsx', index=False)
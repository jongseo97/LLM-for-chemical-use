...
# 모델 로딩 (lora)
model = PeftModel.from_pretrained(model, '/home/jspark/projects/FUse/LLM/script/output/llama-3B-model_1113/', torch_dtype= torch.bfloat16, device_map = 'auto')
...
for data in dataset['validation']:
    ...
    user_context = temp_text.split('<|eot_id|><|start_header_id|>user<|end_header_id|>')[1].strip().split('<|eot_id|>')[0].split('Which')[0].strip() + '\nWhich is the best 1 relevant function and use of the molecule?'
    top2_user_context = temp_text.split('<|eot_id|><|start_header_id|>user<|end_header_id|>')[1].strip().split('<|eot_id|>')[0].split('Which')[0].strip() + '\nWhich is the top-2 relevant functions and uses of the molecule?'
    ...
    temp_template_top2 = TEMPLATE.format(context = system_context, question = top2_user_context)
    
    eval_dataset.append(temp_template)
    eval_top_2_dataset.append(temp_template_top2)
...
if True:
    ...
    t2_prompt = eval_top_2_dataset[i]
    print('Pred : ', make_prediction(prompt).split('\n\n')[-1])
    print('Top 3 Pred : ', make_prediction(t2_prompt).split('\n\n')[-1])
...
top_2_predictions = []
model.eval()
for prompt, top_2_prompt in tqdm(zip(eval_dataset, eval_top_2_dataset)):
    with torch.no_grad():
        output = make_prediction(prompt)
        top_2_output = make_prediction(top_2_prompt)
        predictions.append(output)
        top_2_predictions.append(top_2_output)
...
top_2_predicted_data = [parse_data(text) for text in top_2_predictions]

predicted_data = pd.DataFrame(predicted_data)
top_2_predicted_data = pd.DataFrame(top_2_predicted_data)

predicted_data.to_excel('/home/jspark/projects/FUse/LLM/script/predictions/1115-3B/best_prediction.xlsx', index=False)
top_2_predicted_data.to_excel('/home/jspark/projects/FUse/LLM/script/predictions/1115-3B/top_2_prediction.xlsx', index=False)